...
const animate = ({
  ...
  let tar, keys = [], froms = [], tos = [], rafic, tmoid, starttime, restarttime = false;
  const it = {
    dur,
    ease,
    started: false,
  ...
  const on = (target, props) => {
    if (it.started && !it.ended)
      return;
    tar = target;
    keys = Object.keys(props);
    froms = keys.map((key) => tar[key]);
...
export { CLOSE, CONIC, LINEAR, PI, RADIAL, TAU, animate, arc, bg, circle, clear, curve, fill, font, frame, getcanvas, gradient, line, loop, looping, pxratio, rect, settext, shape, stop, stroke, text, vertex };
